...
MY_SD_CARD_IMAGE="pre_only_sd_card.${DEV_BOARD_TYPE}.${MY_SD_CARD_IMAGE}.image"
...
[ -f "${MY_TMP_TAR}/${DEV_BOARD_TYPE}/preloader-mkpimage.bin" ] || {
...
dd if="${MY_TMP_TAR}/${DEV_BOARD_TYPE}/preloader-mkpimage.bin" of=${MY_LOOP_DEV}p3 2> /dev/null || { 
	echo "ERROR"
	umount ${MY_SD_FAT_MNT}
	losetup -d ${MY_LOOP_DEV}
	rm -Rf ${MY_SD_FAT_MNT} ${MY_TMP_TAR}
