...
int main(int argc, char *argv[])
{
	...
	/*
	 * remove any existing output files from /www/pages/
	 */
	int_result = remove("/www/pages/samples.js");
	if (int_result == -1)
		if( errno != ENOENT)
			error_at_line(1, 0, __FILE__, __LINE__, "while removing file");

	int_result = remove("/www/pages/samplesi.js");
	if (int_result == -1)
		if( errno != ENOENT)
			error_at_line(1, 0, __FILE__, __LINE__, "while removing file");

	int_result = remove("/www/pages/source.js");
	if (int_result == -1)
		if( errno != ENOENT)
			error_at_line(1, 0, __FILE__, __LINE__, "while removing file");

	int_result = open("/www/pages/samples.js", O_RDONLY | O_SYNC);
	if (int_result == -1)
		if( errno != ENOENT)
			error_at_line(1, 0, __FILE__, __LINE__, "failed to remove file '/www/pages/samples.js'");

	int_result = open("/www/pages/samplesi.js", O_RDONLY | O_SYNC);
		if( errno != ENOENT)
			error_at_line(1, 0, __FILE__, __LINE__, "failed to remove file '/www/pages/samplesi.js'");

	int_result = open("/www/pages/source.js", O_RDONLY | O_SYNC);
		if( errno != ENOENT)
			error_at_line(1, 0, __FILE__, __LINE__, "failed to remove file '/www/pages/source.js'");
	...
	int_result = snprintf(command_string, MAX_COMMAND_LENGTH, "%s %s %s >&%d 2>&1", "./fpga_fft", "1", "256", stdout_pipefd[PIPE_WRITE_FD]);
	...
	i = ssize_t_result;
	ssize_t_result = write(stdin_pipefd[PIPE_WRITE_FD], command_output, i);
	if (ssize_t_result < 0)
	...
	int_result = strncmp("47464ee1a0f0e94f84eab724a5904b1f84697e3bd82ee5001a6d6b8cc121610b", command_output, 64);
	if (int_result != 0)
		error_at_line(1, 0, __FILE__, __LINE__, "invalid output from fpga_fft program");

	int_result = snprintf(command_string, MAX_COMMAND_LENGTH, "%s %s >&%d 2>&1", "sha256sum", "/www/pages/samples.js", stdout_pipefd[PIPE_WRITE_FD]);
	if (int_result >= MAX_COMMAND_LENGTH)
		error_at_line(1, 0, __FILE__, __LINE__, "command string truncated");
	...
	int_result = strncmp("eafb8930b93bfccb3ae33c19df7cc7589cfc685a17f3a5ac770b0f29041b433f", command_output, 64);
	if (int_result != 0)
		error_at_line(1, 0, __FILE__, __LINE__, "invalid output from fpga_fft program '/www/pages/samples.js'");

	int_result = snprintf(command_string, MAX_COMMAND_LENGTH, "%s %s >&%d 2>&1", "sha256sum", "/www/pages/samplesi.js", stdout_pipefd[PIPE_WRITE_FD]);
	if (int_result >= MAX_COMMAND_LENGTH)
		error_at_line(1, 0, __FILE__, __LINE__, "command string truncated");
	...
	int_result = strncmp("8c22b5c368434662376d0a2fcb3bd347e643427b1b4ecb486c6c494ea124f2b6", command_output, 64);
	if (int_result != 0)
		error_at_line(1, 0, __FILE__, __LINE__, "invalid output from fpga_fft program '/www/pages/samplesi.js'");

	int_result = snprintf(command_string, MAX_COMMAND_LENGTH, "%s %s >&%d 2>&1", "sha256sum", "/www/pages/source.js", stdout_pipefd[PIPE_WRITE_FD]);
	if (int_result >= MAX_COMMAND_LENGTH)
		error_at_line(1, 0, __FILE__, __LINE__, "command string truncated");
	...
	int_result = strncmp("01f582ed7bfa2635e0f70b369ccd53f24e8b83a51f34420e86a6a4fda85fe977", command_output, 64);
	if (int_result != 0)
		error_at_line(1, 0, __FILE__, __LINE__, "invalid output from fpga_fft program '/www/pages/source.js'");
	...
	int_result = snprintf(command_string, MAX_COMMAND_LENGTH, "%s %s %s %s %s %s > /dev/null 2>&1", "cp", argv[0], "fft.bin", "fpga_fft", "validation_signature.bin", "validation_archive");
	if (int_result >= MAX_COMMAND_LENGTH)
		error_at_line(1, 0, __FILE__, __LINE__, "command string truncated");
