...
overlay.make_all: overlay.extract toolchain.extract linux.build linux.modules
	$(MAKE) -C sw_src LINUX_BRANCH=$(LINUX_BRANCH) $(LINUX_VARIABLES)  INSTALL_DIR=$(CURDIR)/overlay all 2>&1 | tee logs/$$(notdir $$@).log
...
overlay.make_install: overlay.make_all linux.modules_install
	$(MAKE) -C sw_src LINUX_BRANCH=$(LINUX_BRANCH) $(LINUX_VARIABLES) INSTALL_DIR=$(CURDIR)/overlay install

HELP_TARGETS += overlay.make_clean
overlay.make_clean.HELP := Clean custom apps to overlay directory
.PHONY: overlay.make_clean
overlay.make_clean:
	$(MAKE) -C sw_src LINUX_BRANCH=$(LINUX_BRANCH) $(LINUX_VARIABLES) INSTALL_DIR=$(CURDIR)/overlay clean
