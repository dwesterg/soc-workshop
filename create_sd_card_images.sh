...
MY_LOOP_DEV=$(losetup --show -f --partscan ${MY_SD_CARD_IMAGE}) || { echo "ERROR"; exit 1; }
...
dd if="${MY_SD_FAT_MNT}/${DEV_BOARD_TYPE}/preloader-mkpimage.bin" of=${MY_LOOP_DEV}p3 2> /dev/null || { echo "ERROR"; exit 1; }
...
mkfs -t ext3 ${MY_LOOP_DEV}p2 > /dev/null 2>&1 || { echo "ERROR"; exit 1; }

echo "Detaching SD card image file from loop device."
losetup -d ${MY_LOOP_DEV} || { echo "ERROR"; exit 1; }

echo "Removing temporary working directories."
rm -Rf ${MY_SD_FAT_MNT} ${MY_TMP_TAR} || { echo "ERROR"; exit 1; }
